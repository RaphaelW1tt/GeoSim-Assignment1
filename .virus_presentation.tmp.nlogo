...
;;'Slider #1' for the max. capacity of the hospital - the maximum # of people who can get admitted in a hospital at any given time (assuming all hospitals have the same capacity) - put in globals
;;'Slider #2' for the area it can cater ("catchment area") - people from outside the catchment area cannot get into a hospital - put in globals
...
to setup
  clear-all
  setup-constants
  create-persons number-people
  [ setxy random-xcor random-ycor
    set shape "person"
    set sick? false                ;; if true, the turtle is infectious
    set remaining-immunity 0   ;; how many weeks of immunity the turtle has left
    set sick-time 0            ;; how long, in weeks, the turtle has been infectious
...
;to setup-turtles
;to create-persons number-people
;    [ setxy random-xcor random-ycor
;      set age random lifespan
;      set sick-time 0
...
to update-global-variables
  if count persons > 0
    [ set %infected (count persons with [ sick? ] / count persons) * 100
      set %immune (count persons with [ immune? ] / count persons) * 100 ]
end
...
to infect ;; turtle procedure
  ask other persons-here with [ not sick? and not immune? ]
...
to reproduce
  if count persons < carrying-capacity and random-float 100 < chance-reproduce
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy ] ]
...
"" ""
PENS
"sick" 1.0 0 -2674135 true "" "plot count persons with [ sick? ]"
"immune" 1.0 0 -7500403 true "" "plot count persons with [ immune? ]"
"healthy" 1.0 0 -10899396 true "" "plot count persons with [ not sick? and not immune? ]"
"total" 1.0 0 -13345367 true "" "plot count persons"
